#!/usr/bin/env python3
"""Build the final M0 audit matrix, and recompute its summary from the rows.

The skeleton is written once, before substantive execution, with every row NOT_STARTED. Results
arrive as append-only records in RESULTS.jsonl; the matrix is then rebuilt from the rows plus those
records, so the published artifact is always a function of the recorded executions and never a
...
def evidence_resolves(reference: str) -> bool:
    ...
    if kind in ("command", "row", "test", "execution"):
        return bool(value)
    return False
...
def build() -> dict:
    ...
    return {
        ...
        "createdAt": _now(),
        "note": (
            "Rows are derived from the repository and from the audit mandate. Results are merged "
            "from the append-only execution record; no row is edited by hand."),
