...
BASE = "3f730dca1a12245ee8fdf8dfad7a523ed5c1c186"

REASONS = {
    ".iacode/anchors/checkpoint-chain.json":
        "The twelfth integrity anchor, for SETUP-00-CP-0012, which its own checkpoint could not "
        "write and which its successor owes it.",
    ".iacode/attestations/M0-CP-0013.json":
...
CHECKPOINT_REASONS = {
    ...
    "EVIDENCE-RESOLUTION-SUBJECT.json":
        "Independent resolution of every evidence reference the subject declares.",
    "EXPECTED-SET-SUBJECT.json":
        "Independent derivation of the subject's expected requirement set, compared with the set "
        "it declares.",
    ...
    "MEMORY-MEASUREMENT.json":
        "Independent measurement of the engineering memory and the guardrail registry.",
    "MILESTONE-REPORT.md": "The milestone verdict and what supports it.",
    "MIRROR-SEMANTICS-PROBE.json":
        "This audit's own execution of the real mirror over six applicability states.",
    "NA-ESCAPE-PROBE.json":
        "This audit's own probe of the reverse escape the CP11-F-001 repair could have opened.",
...
HARNESS_REASONS = {
    "EXECUTIONS.jsonl": "Append-only record of executions, which AUDIT-EXECUTIONS.json is built from.",
    ...
    "attack_battery_refreshed.py":
        "The variants that re-derive the declared hashes so a refusal is attributable to the "
        "control under test rather than to the inventory binding.",
    "clean_clone.sh": "Re-runs the mandatory validations and the suite in a fresh clone.",
    "complete_matrix.py": "Completes the closure requirements of this checkpoint from audit evidence.",
    ...
    "matrix_rows.py": "The dimensions of the audit mandate, written before execution.",
    "probe_mirror.py": "This audit's own execution of the real mirror over six applicability states.",
    "probe_na_escape.py": "This audit's own probe of the reverse escape.",
    ...
    "verify_memory.py": "Independent measurement of the memory and the guardrail registry.",
}
