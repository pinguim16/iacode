#!/usr/bin/env python3
"""M1-F-003 acceptance: every sealed checkpoint of M1 validates from its own tag, published history only.

Adapted from the M1 audit's harness (GATE-3-CP-0002). The subjects are every sealed checkpoint of
the milestone — the four Gates, the superseded GATE-2-CP-0001, and the audit checkpoint — and the
default source is a clone of the authorised remote, because the finding was precisely that a clone
of this working repository carried objects the remote does not.
...
relies on.
    ...
    python sealed_subjects.py --remote --report ../SEALED-SUBJECTS-PUBLISHED.json
"""
...
SUBJECTS = (
    ...
    ("GATE 3", "GATE-3-CP-0002", "REWORK_REQUIRED", False),
)
