#!/usr/bin/env python3
"""M1-F-003 acceptance: a clean clone of the authorised remote validates and verifies on its own.

Adapted from the M1 audit's harness (GATE-3-CP-0002), where this same execution failed 30 of 31
stages on the published history's gate:tests.
...
def main() -> int:
    ...
    steps = []
    scratch = Path(tempfile.mkdtemp(prefix="iacode-cp0003-clone-"))
